...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if (nargin<4)
        maxMem = 2; % If the user does not input a value for maxMem, set it to 2 gigabytes?
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    

    M = size(R,1); %Number of Reference Points
    N = size(Q,1); %Number of Query Points
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ds = zeros(N,k); %List of the distances of the k-nearest neigbhors from each of the N points
    inds = zeros(N,k);%List of the indices of the corresponding k nearest neighbors.

%Notsure what this is for
    Nr = sum(R.^2,2);
    Nq = sum(Q.^2,2);
%%%%%%%%%%%%%%%%%%%%%%%%%%
    for b = 1:blocks
